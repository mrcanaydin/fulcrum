...
Fulcrum applications are production-ready only when the framework checks and the application deployment checks both pass. The framework should provide safe primitives; the application and platform still own infrastructure policy.

Minimum framework release gates:

- `composer validate --strict`
- `./vendor/bin/pest`
- `php -d memory_limit=-1 ./vendor/bin/phpstan analyse`
- CI integration probes for PostgreSQL, MySQL, and Redis
- Skeleton Docker smoke test with build, migrations, seeders, queue worker, health checks, and a GraphQL query
- Schema validation and, for published APIs, schema diff against the deployed baseline
...
- Queue workers supervised separately from web workers and stopped gracefully during deploys.
- `GET /health/live` and `GET /health/ready` wired to liveness and readiness probes.
- Logs shipped from the configured log destination and correlated with request IDs.
- Token TTLs, trusted proxies, CORS origins, body-size limits, and rate limits reviewed for the application.
- Object storage, SMTP, notification webhooks, and backup/restore procedures verified before launch.
...
- Use `Fulcrum\Database\Migrations\SchemaSupport` for common driver-aware fragments in framework and package migrations.
- Use `BIGINT GENERATED BY DEFAULT AS IDENTITY` on PostgreSQL and `BIGINT UNSIGNED AUTO_INCREMENT` on MySQL for generated primary keys.
- Use `TEXT` for serialized payloads, JSON strings, abilities, and exception bodies that may exceed `VARCHAR(255)`.
- Use `INTEGER`/`INT` epoch timestamps for queue scheduling fields and nullable `TIMESTAMP` columns for application timestamps.
- Create PostgreSQL indexes with separate `CREATE INDEX IF NOT EXISTS` statements; MySQL can define keys inline in `CREATE TABLE`.
- Drop PostgreSQL unique indexes with `DROP INDEX` unless the index was created as a table constraint.
- Keep database-backed queue reservation portable by claiming jobs with a conditional `UPDATE ... WHERE reserved_at IS NULL` instead of requiring engine-specific lock syntax.

CI runs migration, rollback, insert ID, transaction, Redis, and database queue probes against real PostgreSQL and MySQL services. Local MySQL probes require the `pdo_mysql` PHP extension; PostgreSQL probes require `pdo_pgsql`.
